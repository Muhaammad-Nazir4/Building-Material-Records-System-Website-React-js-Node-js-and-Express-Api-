...
function DealerStockSpecificProduct() {
  ...
  useEffect(()=>{
    ...
    async function fetchData() {
      ...
      try {
        const response = await fetch(`http://localhost:4000/GetProductForDealerStock?ProductId=${urlParams.get('ProductId')}&UserId=${localStorage.getItem('UserId')}`);
        const data = await response.json();
        setProduct(data);
        console.log(data)
    ...
    //     const response = await fetch(`http://localhost:4000/GetCompanyReviews?ProductId=${urlParams.get('ProductId')}`);
    //     const data = await response.json();
    //     setReviews(data);
    //     console.log(data)
    //   } catch (error) {
  ...
  return (
    ...
    <Modal
          ...
          <MDBCard >
          <MDBCardBody>
            {/* <form enctype='multipart/from-data'  action={`http://localhost:4000/CompanyUpdateProduct?ProductId=${ProductId}`} method='post' > */}
            {console.log("ids",ids)}
            <form action={`http://localhost:4000/UpdateDealerStockPrice?ProductId=${ids}&UserId=${localStorage.getItem('UserId')}&Price=${product.ProdPrice}`} encType='multipart/form-data' method="post">
      ...
      <Modal
          ...
          <MDBCardBody>
            ...
            {console.log("ids",ids)}
            {/* Product[0].Quantity */}
            <form action={`http://localhost:4000/UpdateDealerThreshold?ProductId=${ids}&UserId=${localStorage.getItem('UserId')}`} method="post">
