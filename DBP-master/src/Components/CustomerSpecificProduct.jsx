...
function CustomerSpecificProduct() {
  ...
  function submitpayment(action){
    ...
    // window.location.href = action=`http://localhost:4000/DealersOrder?UserId=${localStorage.getItem('UserId')}&ProductId=${ids}&CVV=${Payment.CVV}&AccountNo=${Payment.CardNumber}&Month=${Payment.Month}&Year=${Payment.Year}&Price=${Price}&Quantity=${Quantity}`;
  }
  ...
  useEffect(()=>{
    ...
    async function fetchData() {
      try {
        const response = await fetch(`http://localhost:4000/GetProductReviews?ProductId=${urlParams.get('ProductId')}`);
      ...
      try {
        console.log(UserId);
        const response = await fetch(`http://localhost:4000/GetProductRating?ProductId=${urlParams.get('ProductId')}`);
        const data = await response.json();
      ...
      try {
        console.log(UserId);
        const response = await fetch(`http://localhost:4000/GetDealerRating?StockId=${urlParams.get('StockId')}`);
        const data = await response.json();
        console.log(data[0].Rating)
        if(data[0].Rating !== null){
      ...
      try {
        const response = await fetch(`http://localhost:4000/GetProductusingIdDealer?ProductId=${urlParams.get('ProductId')}`);
        const data = await response.json();
        setProduct(data);
        console.log("Product",data)
        console.log(data[0].CompanyId)
        const response1 = await fetch(`http://localhost:4000/GetCompanyRating?UserId=${data[0].CompanyId}`);
      ...
      try {
        console.log(UserId);
        const response = await fetch(`http://localhost:4000/GetDealerProfile?UserId=${localStorage.getItem('UserId')}`);
        const data = await response.json();
        setProfile(data);
        console.log("Profile",data)
      } catch (error) {
      ...
      try {
        const response = await fetch(`http://localhost:4000/GetDealerInfoForCustomer?StockId=${urlParams.get('StockId')}`);
        const data = await response.json();
        setdealer(data);
  ...
  return (
    ...
    <MDBContainer fluid className="my-5">
      ...
      <MDBRow className='g-0'>
        <MDBCol md='4'>
          <MDBCardImage src={`http://localhost:4000/images/${prof.ProductPic}`} alt='...' fluid />
        </MDBCol>
        <MDBCol md='8'>
          <MDBCardBody>
          <MDBCardTitle>{"Brand: "+prof.CompanyName}</MDBCardTitle>
            ...
            <MDBRow >
              <MDBCol md="6" lg="6" className="mb-3">
                <button type="button" className="btn btn-primary btn-block mb-4" onClick={()=>{const urlParams = new URLSearchParams(window.location.search);window.location.href = `http://localhost:4000/CustomerAddToCart?ProductId=${prof.ProductId}&UserId=${localStorage.getItem('UserId')}&StockId=${urlParams.get('StockId')}`}} style={{maxWidth:"150px"}}>Add To Cart</button>
              </MDBCol>
              <MDBCol md="6" lg="6" className="mb-3" >
      ...
      <MDBRow className='g-0'>
        <MDBCol md='4'>
          <MDBCardImage src={`http://localhost:4000/images/${prof.Product}`} alt='...' fluid />
        </MDBCol>
        <MDBCol md='8'>
      ...
      {dealer.map((dea,index)=>(
        ...
        <MDBCol md='4'>
            <MDBCardImage src={`http://localhost:4000/images/${dea.Picture}`} alt='...' fluid />
        </MDBCol>
        </MDBRow>
        ))}
              <MDBCardBody>
    ...
    <Modal
          ...
          >
            ...
            <form action={`http://localhost:4000/CustomerOrder?UserId=${localStorage.getItem('UserId')}&DealerId=${localStorage.getItem('DealerId')}&ProductId=${ids}&Year=${Address.Year}&Price=${Price}&Quantity=${Quantity}&StockId=${localStorage.getItem('StockId')}`} method='post' id='CustomerOrder'>
      ...
      <Modal
          ...
          >
            ...
            <form  action={`http://localhost:4000/DealersOrder?UserId=${localStorage.getItem('UserId')}&ProductId=${ids}&Year=${Payment.Year}&Price=${Price}&Quantity=${Quantity}`} method="post" id = "DealersOrder">
            <MDBCard className='d-flex'>
            <MDBCardBody className='p-5'>
