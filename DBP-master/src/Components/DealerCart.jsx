    ...
    if(urlParams.get('show') === '1'){
      setshowpayment(true);
        ...
        async function fetchData() {
          try {
            const response = await fetch(`http://localhost:4000/GetDealerCart?UserId=${localStorage.getItem('UserId')}`);
  ...
  return (
            ...
            <MDBCardBody>
              <MDBRow>
                <MDBCol md="12" lg="3" className="mb-4 mb-lg-0">
                  ...
                  >
                    <MDBCardImage
                      src={`http://localhost:4000/images/${prof.ProductPic}`}
                      fluid
                      className="w-100"
                    />
                ...
                >
                  ...
                  <div className="d-flex flex-column mt-4">
                    ...
                    <button type="button" size="sm" className="btn bg-warning btn-block mb-4 mt-20"
                     style={{fontSize: '15px'}} onClick={()=>{window.location.href=`http://localhost:4000/RemoveDealerCartProduct?UserId=${localStorage.getItem('UserId')}&ProductId=${prof.ProductId}`}}>Remove From Cart</button>
                  </div>
                </MDBCol>
    ...
    <Modal
          ...
          >
            ...
            <form action={`http://localhost:4000/UpdateDealerCartDuringPaymentCart?UserId=${UserId}&AddressId=${Profile[0].AddressId}&ProductId=${localStorage.getItem('Id')}`} method='post' id='UpdateDealerAddress'>
      ...
      <Modal
          ...
          >
            ...
            <form  action={`http://localhost:4000/DealerOrderFromCart?UserId=${localStorage.getItem('UserId')}&ProductId=${localStorage.getItem('Id')}&Year=${Payment.Year}&Price=${localStorage.getItem('Price')}&Quantity=${Quantity}`} method="post" id = "DealersOrder">
            <MDBCard className='d-flex'>
            <MDBCardBody className='p-5'>
