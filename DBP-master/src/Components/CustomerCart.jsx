  ...
  const [Price, setPrice] = useState(0);
    ...
    useEffect(()=>{
        ...
        async function fetchData() {
          try {
            const response = await fetch(`http://localhost:4000/GetCustomerCart?UserId=${localStorage.getItem('UserId')}`);
  ...
  return (
                ...
                >
                  ...
                  <div className="d-flex flex-column mt-4">
                    ...
                    <button type="button" size="sm" className="btn bg-warning btn-block mb-4 mt-20"
                     style={{fontSize: '15px'}} onClick={()=>{window.location.href=`http://localhost:4000/RemoveCustomerCartProduct?UserId=${localStorage.getItem('UserId')}&StockId=${prof.StockId}`}}>Remove From Cart</button>
                  </div>
                </MDBCol>
    ...
    <Modal
          ...
          >
            ...
            <form action={`http://localhost:4000/CustomerOrderInCart?UserId=${localStorage.getItem('UserId')}&DealerId=${localStorage.getItem('DealerId')}&ProductId=${localStorage.getItem("Id")}&Year=${Address.Year}&Price=${localStorage.getItem("Price")}&Quantity=${Quantity}&StockId=${localStorage.getItem('StockId')}`} method='post' id='CustomerOrder'>
