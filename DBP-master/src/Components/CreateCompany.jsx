...
function CreateCompany() {
  ...
  useEffect(()=>{;
    ...
    async function fetchData() {
      ...
      try {
        const response = await fetch("http://localhost:4000/GetCompanyType");
        const data = await response.json();
        setdata(data);
        console.log(data);
      } catch (error) {
      ...
      <MDBRow>
        ...
        <MDBCol md='7' >
          ...
          <MDBCard className='my-5 d-flex'>
            ...
            <form action={`http://localhost:4000/CreateCompany?UserId=${UserId}&AddressId=${AddressId}&option=${option}`} method='post' id="signupform">
