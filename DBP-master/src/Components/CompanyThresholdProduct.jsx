...
function CompanyThreshold() {
  ...
  useEffect(()=>{
    ...
    async function fetchData() {
      try {
        const response = await fetch(`http://localhost:4000/GetThresholdCompany?UserId=${localStorage.getItem("UserId")}`);
  ...
  return (
    ...
    <Modal
          ...
          <MDBCardBody>
            ...
            {console.log("ids",ids)}
            {/* Product[0].Quantity */}
            <form action={`http://localhost:4000/CompanyAddStockInThreshold?ProductId=${ids}&quantity=${quantity}`} method="post">
