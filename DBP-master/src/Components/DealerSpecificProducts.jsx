...
function DealerSpecificProduct() {
  ...
  useEffect(()=>{
    ...
    async function fetchData() {
      try {
        const response = await fetch(`http://localhost:4000/GetProductReviewsInDealer?ProductId=${urlParams.get('ProductId')}`);
      ...
      try {
        console.log(UserId);
        const response = await fetch(`http://localhost:4000/GetProductRatingInDealer?ProductId=${urlParams.get('ProductId')}`);
        const data = await response.json();
...
const[showpayment,setshowpayment] = useState(false);
  ...
  return (
    ...
    <MDBContainer fluid className="my-5">
          ...
          <MDBCardTitle>{"Brand: "+prof.CompanyName}</MDBCardTitle>
            ...
            <MDBRow >
              <MDBCol md="6" lg="6" className="mb-3">
                <button type="button" className="btn btn-primary btn-block mb-4" onClick={()=>{window.location.href = `http://localhost:4000/DealerAddToCart?ProductId=${prof.ProductId}&UserId=${localStorage.getItem('UserId')}`}} style={{maxWidth:"150px"}}>Add To Cart</button>
              </MDBCol>
              <MDBCol md="6" lg="6" className="mb-3" >
      ...
      <MDBRow className='g-0'>
        <MDBCol md='4'>
          <MDBCardImage src={`http://localhost:4000/images/${prof.Picture}`} alt='...' fluid />
        </MDBCol>
        <MDBCol md='8'>
    ...
    <Modal
          ...
          >
            ...
            <form action={`http://localhost:4000/UpdateDealerAddressDuringPayment?UserId=${UserId}&AddressId=${Profile[0].AddressId}&ProductId=${ids}`} method='post' id='UpdateDealerAddress'>
