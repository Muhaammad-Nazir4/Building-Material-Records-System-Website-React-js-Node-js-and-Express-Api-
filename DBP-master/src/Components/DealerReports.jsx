...
const DealerReports = () => {
  ...
  async function fetchData() {
    ...
    if(document.getElementById("selectedItem").value === "Monthly Product Percent Report"){
      try {
        const response =await fetch(`http://localhost:4000/MonthlyProductWisePercentDealerReport?UserId=${localStorage.getItem('UserId')}`);
    ...
    if(document.getElementById("selectedItem").value === "Monthly Product Profit Report"){
      try {
        const response =await fetch(`http://localhost:4000/MonthlyProductWiseProfitDealer?UserId=${localStorage.getItem('UserId')}`);
        const data =await response.json();
        setProduct(data);
        setTitle(true)
        console.log(data)
    ...
    if(document.getElementById("selectedItem").value === "Monthly Product Sale Report"){
      try {
        const response =await fetch(`http://localhost:4000/MonthlyProductWiseSaleDealerReport?UserId=${localStorage.getItem('UserId')}`);
        const data =await response.json();
        setProduct(data);
        setTitle(true)
        console.log(data)
    ...
    if(document.getElementById("selectedItem").value === "Products Purchase Report"){
      try {
        const response =await fetch(`http://localhost:4000/DealerPurchaseReport?UserId=${localStorage.getItem('UserId')}`);
        const data =await response.json();
        setProduct(data);
        setTitle(true)
        console.log(data)
    ...
    if(document.getElementById("selectedItem").value === "Products Order Report"){
      try {
        const response =await fetch(`http://localhost:4000/DealerOrderReport?UserId=${localStorage.getItem('UserId')}`);
        const data =await response.json();
        setProduct(data);
        setTitle(true)
        console.log(data)
